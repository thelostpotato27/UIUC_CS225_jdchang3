...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(singleQueue.size() + mergeQueue.size() != 1){
        ...
        int freq_sum = small_1 -> freq.getFrequency() + small_2 -> freq.getFrequency();
        TreeNode *parent = new TreeNode(freq_sum);
        parent -> left = small_1;
        parent -> right = small_2;
        mergeQueue.push(parent);
    }
    TreeNode *root = removeSmallest(singleQueue, mergeQueue);
    root_ = root;
    /**
     * @todo Your code here!
     *
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        if(current -> left == NULL){
            char myChar = current -> freq.getCharacter();
            ss << myChar;
            current = root_;
        }
        /**
...
{
    while (bfile.hasBits()) {
        int next = bfile.getNextBit();
        if (next){
            Frequency info = Frequency(bfile.getNextByte(), 0);
            TreeNode* leaf = new TreeNode(info);
            return leaf;
        }else{
