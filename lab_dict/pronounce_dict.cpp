...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    auto iter1 = dict.find(first_word);
    auto iter2 = dict.find(second_word);

    if(iter1 == dict.end() || iter2 == dict.end()){
        return false;
    }

    vector<string> pronounce_1 = iter1->second;
    vector<string> pronounce_2 = iter2->second;

    if(pronounce_1 == pronounce_2){
        return true;
    }
