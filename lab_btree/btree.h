...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    int m = (l+r)/2;
    while(r-l != 0 && !(elements[m] == val)){
        //check if insert left or insert right of element
        
        if(elements[m] > val){ //search left
            r = m;
        }
        else{ //search right
            l = m+1;
        ...
        m = (l+r)/2;
        //if(r==l && elements[m] < val) return m+1;
    }
    //std::cout << "Insert index is " << m << std::endl;
    return m;
}
