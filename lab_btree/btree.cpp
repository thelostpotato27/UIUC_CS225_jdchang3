...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    unsigned i;
    //recursive case, call find on the correct child
    //std::cout << *subroot << std::endl;
    for(i = 0; i < (subroot->elements).size(); ++i){
        if((subroot->elements)[i] > key){
            V vreturn = find((subroot->children)[i], key);
            //std::cout << "Index " << i << std::endl;
            return vreturn;
        }
    }
    //find in last child node
    //std::cout << "Index " << i << std::endl;
    V vreturn = find((subroot->children)[i], key);
    return vreturn;
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    /* TODO Your code goes here! */
     //set the right node keys
    for(auto start_elem_itr = mid_elem_itr + 1; start_elem_itr != child->elements.end(); ++start_elem_itr){
        (new_right->elements).push_back(*start_elem_itr);
    }
    //set the right node children
    if(child->is_leaf == false){
        for(auto start_child_itr = mid_child_itr; start_child_itr != child->children.end(); ++start_child_itr){
            (new_right->children).push_back(*start_child_itr);
        }
    }
    //insert new key into the parent
    (parent->elements).insert(elem_itr, *mid_elem_itr);

    //insert the new right child in children
    (parent->children).insert(child_itr, new_right);

    //Get rid of the second half of the keys in child
    (new_left->elements).erase(mid_elem_itr, (child->elements).end());
    
    //Get rid of the second half of the children in child
    if(child->is_leaf == false){
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    //if subroot is a leaf node, base case
    //std::cout << *subroot << std::endl;
    if(subroot->is_leaf == true){
    ...
    //check if child is order size, split if so
