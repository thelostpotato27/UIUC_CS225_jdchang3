...
class KDTree
{
  ...
  private:
    void KDTree_help(KDTreeNode* parent, vector<Point<Dim>>& newPoints, unsigned counter);

    void sort(vector<Point<Dim>>& newPoints, unsigned left, unsigned right, unsigned median_index, unsigned dim);

    unsigned partition(vector<Point<Dim>>& newPoints, unsigned left, unsigned right, unsigned dim);

    void deleter(KDTreeNode* current);

    typename KDTree<Dim>::KDTreeNode* copy(const KDTreeNode* other);

    void find_closest(KDTreeNode* traverse_node, Point<Dim>& current_closest, const Point<Dim>& point_to_compare, unsigned dim) const;
